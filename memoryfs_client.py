...
class DiskBlocks():
  def __init__(self,N,localhosts):
    ...
    # connect to the server with corresponding server_url
    ## example server_url: 'http://localhost:8000' 
    self.block_server = {}
    for i in range(N):
      localhost =self.localhosts[i]
      self.block_server[i] = xmlrpc.client.ServerProxy('http://'+localhost, use_builtin_types=True)
    # print(self.block_server)


## Map the virtual address to physical address and corresponding server
## Make sure that the load are seperated evenly with the application of RAID5

  def Map(self, block_number):
    physical_block_number = block_number//(self.N-1)
    server_index = block_number-physical_block_number*(self.N-1)
    parity_server_index = (self.N-1)-physical_block_number%self.N

    if server_index >= parity_server_index:
      server_index = server_index+1
  
    return server_index, physical_block_number, parity_server_index


## bytearrayXor:calculate xor of two bytearray

  def bytearrayXor(self, input1, input2):
    result = {}
    N = len(input1)
    for i in range(N):
...
## faultTolerance
  ...
  def faultTolerance(self,server_index, physical_block_number):
      # acquire data of other servers
      helpdata = []
      for i in range(self.N):
          if i != server_index:
              temp_data = self.block_server[i].Get(physical_block_number)
              temp_bytes = bytearray(temp_data)
              helpdata.append(temp_bytes)

      # recover decay/corruption
      input1 = helpdata[0]
      input2 = helpdata[1]
      result = self.bytearrayXor(input1,input2)
      for j in range(2, self.N-1):
          input3 = helpdata[j]
          temp= self.bytearrayXor(result,input3)
          result = temp
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def Put(self, block_number, block_data):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS): 
      ...
      # Map block_number to physical_block_number
      server_index, physical_block_number, parity_server_index = self.Map(block_number)
      
      # RAID5
      # Read1
      oldData = self.Get(block_number)
      # ljust does the padding with zeros
      newdata = bytearray(block_data.ljust(BLOCK_SIZE,b'\x00'))

      # Read2
      oldParity = self.block_server[parity_server_index].Get(physical_block_number)

      oldParity = bytearray(oldParity)
      
      # Calculate parity
      temp = self.bytearrayXor(newdata,oldData)
      newParity = self.bytearrayXor(temp,oldParity)

      # Write 1
      # Put new data into localhosts[server_index]'s physical_block_number
      # Write block to server
      # self.block[block_number] = putdata
      ret = self.block_server[server_index].Put(physical_block_number,newdata)
      ...
      # Write 2
      # Put new parity into localhosts[parity_server_index]'s physical_block_number
      putdata = bytearray(newParity.ljust(BLOCK_SIZE,b'\x00'))
      ret = self.block_server[parity_server_index].Put(physical_block_number,putdata)
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def Get(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      # Acquire data from corresponding server
      # With the ability to detect that server is down and fault tolerance
      data = self.block_server[server_index].Get(physical_block_number)
      
      # Fault-tolerance of block's decay/corruption
      data_str = str(data)
      check = bytearray(data_str,'utf-8')
      # print(check)
      if check.decode() == 'istilltry':
          data = self.faultTolerance(server_index, physical_block_number)
      # Fault tolerance of wrong checksum
      if check.decode() == 'error':
          data = self.faultTolerance(server_index, physical_block_number)
